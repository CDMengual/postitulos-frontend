...
export default function UsuariosTable({ data, loading, onEdit, onDelete }: Props) {
  ...
  const handleView = () => {
    if (selectedRow) {
      router.push(`/gestion/usuarios/${selectedRow.id}`);
    }
    handleMenuClose();
  };
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      flex: 0.8,
      minWidth: 100,
      valueGetter: (_, row: User) => row.instituto?.nombre || "-",
    },
    {
      field: "distrito",
      headerName: "Distrito",
      flex: 1,
      minWidth: 120,
      valueGetter: (_, row: User) => row.instituto?.distrito?.nombre || "-",
    },
    {
      field: "rol",
      headerName: "Rol",
