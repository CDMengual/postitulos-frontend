...
export default function InscripcionesTable({
  ...
  const columns: GridColDef<InscripcionListadoItem>[] = [
        ...
        return (
          <Select
            ...
            onChange={(event: SelectChangeEvent<string>) => {
              ...
              onInstitutoChange(row.id, raw === "" ? null : Number(raw));
            }}
            sx={{ minWidth: 180 }}
          >
      ...
      renderCell: ({ row }: GridRenderCellParams<InscripcionListadoItem>) => {
        ...
        return (
          <PillMenu
            ...
            onChange={(value) => onEstadoChange(row.id, value as EstadoInscripcionPrivada)}
          />
        );
      },
    },
    {
      ...
      sortable: false,
      renderCell: ({ row }: GridRenderCellParams<InscripcionListadoItem>) => {
        const options = DOCUMENTACIONES.map((documentacion) => {
        ...
        return (
          <PillMenu
            ...
            disabled={isRowSaving(row.id)}
            onChange={(value) =>
              onDocumentacionChange(row.id, value as DocumentacionInscripcion)
            }
          />
        );
      },
    ...
    },
    {
      ...
      renderCell: ({ row }: GridRenderCellParams<InscripcionListadoItem>) => (
        <Tooltip title="Ver detalle">
          <IconButton
            size="small"
            onClick={() => router.push(`/gestion/inscripciones/${row.id}`)}
          >
