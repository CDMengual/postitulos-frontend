...
import { Formulario, CampoFormulario } from "@/types/formulario";
    ...
    try {
      ...
      for (const [key, file] of fileEntries) {
        ...
        const signRes = await api.post<UploadSignResponse>(
          `/public/cohortes/${cohorte.id}/uploads/sign`,
          {
            dni,
            tipo,
            fileName: file.name,
        ...
        if (!uploadUrl || !path) {
          appToast.error("No se pudo obtener firma de subida");
          return;
        }
        ...
        if (!uploadRes.ok) {
          appToast.error("No se pudo subir uno de los archivos");
          return;
        }

        uploadedFilePaths[key] = path;
      ...
      Object.entries(formData).forEach(([key, value]) => {
        ...
        if (value instanceof File) {
          datosFormulario[key] = uploadedFilePaths[key] || null;
          return;
        }
      ...
      await api.post(`/public/cohortes/${cohorte.id}/inscripciones`, {
